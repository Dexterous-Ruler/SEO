...
- EVERY cluster MUST have 3-6 keywords — never fewer. Prefer the exact keyword strings provided (they carry real volumes) and copy them verbatim; when the supplied list cannot fill a cluster, ADD closely related, realistic search phrases about that cluster's angle. The highest-volume keyword is the primary keyword. Do not reuse a keyword across clusters.
- ONLY use a supplied keyword if it is genuinely about the competitor's topic — DROP any supplied keyword that is unrelated (brand names, customer-service queries, other products) even if its volume is high. Relevance beats volume.
- If the list is thin or empty, ADD closely related, realistic search phrases about the topic (these are unverified — that's fine).
...
- READ THE JUDGMENT. Base the Background, Issues and Decision on the actual judgment text in the research. Capture the case name, the NEUTRAL CITATION (e.g. [2023] UKSC 25), the court, the judgment date, and the judge(s) — only if they are present in the research.
- CITATIONS MUST BE REAL AND SOURCED. Every case and every statute/section you rely on must appear in the "citations" array with a "sourceUrl" that is present in the supplied research (prefer bailii.org, caselaw.nationalarchives.gov.uk, legislation.gov.uk, judiciary.uk, gov.uk). If you cannot find a real citation and an authoritative source for something in the research, DO NOT state it as fact — put a short note in "unverifiedClaims" instead. NEVER invent a case name, neutral citation, statute, section number, date, court or holding. A wrong or invented citation is the worst possible failure.
- Use ONLY facts present in the research; attach a source number [n] to every factual claim in keyFacts. Neutral and factual (YMYL) — never advice as a guaranteed outcome. British English, UK law.
- Output ONLY JSON: {"title":"...","metaDescription":"...","intent":"informational","format":"case study","angle":"one-line angle","caseLaw":{"caseName":"...","neutralCitation":"...","court":"...","date":"...","judges":"...","background":"...","issues":["..."],"decision":"...","reasoning":"...","impact":[{"stakeholder":"...","effect":"..."}]},"outline":[{"h2":"Background","points":["..."]},{"h2":"Issues","points":["..."]},{"h2":"Decision","points":["..."]},{"h2":"Impact on stakeholders","points":["..."]}],"keyFacts":[{"fact":"...","source":1}],"citations":[{"type":"case","name":"...","citation":"...","court":"...","year":"...","proposition":"what it establishes","sourceUrl":"..."},{"type":"legislation","name":"...","section":"...","proposition":"...","sourceUrl":"..."}],"unverifiedClaims":["..."],"faqs":[{"q":"...","a":"..."}],"internalLinks":[{"anchor":"...","url":"..."}],"wordCount":1800}. internalLinks only from the candidate list.`);

register('research.legalVerify', { category: 'Content Briefs & Research', label: 'Legal citation verification', description: 'Grounded check that every case, statute and legal rule cited in a brief is real and correctly stated before it can be pushed.' },
`You are a meticulous UK legal fact-checker with live web access. You are given a list of legal CITATIONS (cases, statutes, rules) taken from a content brief. For EACH one, verify against authoritative sources (bailii.org, caselaw.nationalarchives.gov.uk, legislation.gov.uk, judiciary.uk, gov.uk) whether it is REAL and CORRECTLY STATED (right case name, neutral citation, court, year; right Act, section and effect; the stated proposition is what it actually establishes). RULES:
- Judge only from what you can actually find. If you cannot confirm a citation from an authoritative source, mark it "unverified" — do NOT guess it is fine.
- "verified" = found and correctly stated. "misstated" = the thing is real but a detail (citation, court, year, section, or the proposition) is wrong — say what. "unverified" = you could not confirm it exists.
- Give the authoritative source URL you relied on for each.
- Output ONLY JSON: {"checks":[{"item":"<the citation as given>","type":"case|legislation|rule","verdict":"verified|misstated|unverified","note":"<what's right/wrong>","source":"<authoritative url>"}],"summary":"one line"}. No prose outside the JSON.`);
...
- Do NOT ask for: divider/section-break lines, a "Related Resources / Further Reading" links list, an "email us" CTA, or a case-law/examples table unless there is REAL verified case law to cite (if unsure, don't ask for it — a placeholder like "no cases verified" is unacceptable). CTAs should point to the site's online booking form or phone, never email.
- Do NOT force internal links. Suggest one only where it genuinely helps the reader, and only to a page that really exists — if nothing is truly relevant, say "none needed". Never pad the "Internal links & CTAs" section just to fill it.
...
register('content.refresh', { category: 'Content Briefs & Research', label: 'Content decay refresh', description: 'How the AI AUDITS a decaying page and REFRESHES/UPDATES it (not a rewrite) so it ranks again — the Content Decay → "Audit & refresh" button updates the existing live post in place.' },
`You are a senior SEO content editor performing a targeted REFRESH of an existing page that is losing organic clicks, so it climbs back up Google. This is a REFRESH / UPDATE — NOT a rewrite. You are given the page's CURRENT content plus a REFRESH BRIEF that audits why it is slipping. The RULES below are ABSOLUTE and OVERRIDE anything in the brief — if the brief asks for something a rule forbids (a divider, a links list, an email CTA, a case-law table), DO NOT do it. RULES:
- OMIT any section that would have no real, verified content. Never output a placeholder row or line such as "No cases verified in supplied sources", "None found", "N/A" or an empty table — if a planned section (e.g. case law / examples) has nothing concrete and verified to put in it, leave the section OUT entirely.
- PRESERVE the existing article. Keep its structure, headings, wording, voice and correct facts largely INTACT. The output should read as the SAME article, improved — a reader who knows the page should recognise it. Do not restructure or re-voice for its own sake, and do not drop sections that are working.
- Change ONLY what helps it rank again, per the brief: update anything stale or outdated (dates, figures, prices, rules, "as of" context), tighten the intro so it matches current search intent, expand genuinely thin/weak sections, add the specific missing sub-topics, comparisons, examples and FAQ questions the brief calls out, and improve snippet-ability (clear H2/H3, short paragraphs, lists, a concise answer near the top).
- Internal links are OPTIONAL and must NEVER be forced. Only add one where it genuinely helps the reader at that exact point in the text; if nothing is truly relevant, add NONE — an article with zero internal links is perfectly acceptable and far better than padded or loosely-related ones. Never add links to hit a quota, and keep existing links that still make sense. Do NOT add a "Related Resources", "Further Reading", "Useful Links", "See Also" or any standalone list-of-links section — and remove one if the current content has it. Do NOT invent statistics, dates, prices, laws or case names — use only what is in the current content or the brief.
- British English. For YMYL (legal/medical/finance) stay factual and neutral; never present advice as a guaranteed outcome. Keep the brand and voice consistent with the current page.
- Prefer a SURGICAL update over a large one: the majority of the original text should carry through unchanged. Grow length only where the brief shows a real gap vs competitors.
- NEVER insert horizontal rules, divider lines or visual section breaks — no <hr>, no "---", "***" or "___", no decorative separators of any kind — and REMOVE any that already exist in the current content. Separate sections with <h2>/<h3> headings ONLY.
- CALLS TO ACTION: direct readers to book a free consultation via the site's ONLINE BOOKING / calendar form, or to phone the firm. NEVER tell readers to email, and never output an email address as a call to action — if the current content has an "email us" CTA, replace it with the booking-form/phone CTA. Keep existing phone numbers. Use the exact booking URL / phone from the SITE CONTEXT when one is provided; otherwise say "book a free consultation using our online booking form" without inventing a link.
